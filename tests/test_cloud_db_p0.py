...
def _resp_batch(ok=True):
    ...
    if ok:
        ...
        m.json.return_value = {"results": [
            {"type": "ok", "response": {"type": "execute", "result": {}}},
            {"type": "error", "error": {"message": "UNIQUE failed"}},
            {"type": "ok", "response": {"type": "execute", "result": {}}},
            {"type": "close", "response": {}},
        ]}
    return m
...
def test_batch_atomico_ok(monkeypatch):
    monkeypatch.setenv("TURSO_DATABASE_URL", "https://db.turso.io")
    monkeypatch.setenv("TURSO_AUTH_TOKEN", "tok")
    with patch("mkt_flow_p0.cloud_db.requests.post", return_value=_resp_batch(True)) as mp:
        r = C.turso_batch([{"sql": "INSERT INTO t VALUES (?)", "args": ("a",)},
                           {"sql": "INSERT INTO t VALUES (?)", "args": ("b",)}])
        assert r["total"] == 2
        reqs = mp.call_args.kwargs["json"]["requests"]
        assert [q["stmt"]["sql"] for q in reqs if q["type"] == "execute"] == [
            "BEGIN", "INSERT INTO t VALUES (?)", "INSERT INTO t VALUES (?)", "COMMIT"]
        assert reqs[-1] == {"type": "close"}


def test_batch_erro_aponta_indice(monkeypatch):
    monkeypatch.setenv("TURSO_DATABASE_URL", "https://db.turso.io")
    monkeypatch.setenv("TURSO_AUTH_TOKEN", "tok")
    with patch("mkt_flow_p0.cloud_db.requests.post", return_value=_resp_batch(False)):
        r = C.turso_batch([{"sql": "INSERT INTO t VALUES (?)", "args": ("a",)}])
        assert r["indice"] == 0 and "UNIQUE" in r["erro"]
