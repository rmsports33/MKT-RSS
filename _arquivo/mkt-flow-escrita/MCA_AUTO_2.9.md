...
## Como atribuir sem repetir (evitar hedge excessivo)

Contenção epistêmica não significa repetir "segundo as informações fornecidas", "de acordo com a descrição", "o material informa" a cada frase — isso soa robótico e reduz a credibilidade do texto, o oposto do objetivo de review genuíno (seção 9). Regra prática:

- atribua a fonte **uma vez**, de forma clara, logo no início do bloco ou da peça (ex.: "segundo o fabricante" ou "conforme a ficha do produto", já na abertura);
- depois da primeira atribuição, escreva o restante do bloco em linguagem direta e natural, sem repetir a expressão de hedge a cada frase — a atribuição inicial já cobre o bloco inteiro;
- reserve o hedge explícito e repetido apenas para um claim específico, isolado, que realmente precise da ressalva individual (ex.: um dado que pode ser mal interpretado fora de contexto) — não para todo parágrafo do texto.

Isso não relaxa o guardrail: o conteúdo continua vindo só do que foi fornecido. Muda apenas a forma de atribuir, não a obrigação de não inventar.
